...
int main() {
    ...
    GLFWwindow *window = glfwCreateWindow(_width, _height, "Colors", NULL, NULL);
    if(window == NULL) {
    ...
    Shader objShader("../src/11_colors/shader/obj.vs.glsl", "../src/11_colors/shader/obj.fs.glsl"), // 物体渲染
    lightShader("../src/11_colors/shader/light.vs.glsl", "../src/11_colors/shader/light.fs.glsl"); // 光源渲染
    ...
    glVertexAttribPointer(0, 3, GL_FLOAT, GL_FALSE, 3 * sizeof(GLfloat), (void*)0); // layout (location = 0) aPos
    ...
    while(!glfwWindowShouldClose(window)) {
        ...
        objShader.use();
        objShader.setVec3("lightColor", glm::vec3(1.0f, 0.0f, 0.0f)); // 需要给物体渲染器设置光源颜色，同时也需要在光源的片元渲染器中设置颜色
        objShader.setVec3("objColor", glm::vec3(1.0f, 1.0f, 1.0f));
        // 假如光是红色的，物体是白色的，那么物体就能吸收除红色以外其余的颜色，只反射无法吸收的红色
